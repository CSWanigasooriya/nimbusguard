...
class LoadGenerator:
            ...
            try:
                async with self.session.get(f"{self.base_url}/process/status") as response:
                    if response.status == 200:
                        ...
                        logger.info(f"📊 Monitor: {memory_objects} objects, {memory_mb}MB, {total_processed} total processed")
                    else:
    ...
    async def run_baseline_comparison_phases(self, load_test: LoadTest) -> Dict[str, Any]:
        ...
        logger.info(f"📋 Pattern: Morning Rush(6min) → Peak Traffic(8min) → Sustained Business(10min) → Evening Cooldown(6min) = 30min")
        ...
        for i, phase in enumerate(phases, 1):
            ...
            logger.info(f"   📊 Production Load: {phase['requests_per_minute']} req/min, CPU={phase['cpu_intensity']}, Memory={phase['memory_size']}MB")
            ...
            if load_test.fire_and_forget:
                ...
                logger.info(f"   🔥 Firing {phase_test.total_requests} requests over {phase['duration_minutes']} minutes (fire-and-forget)...")
                ...
                logger.info(f"   🚀 Firing {phase_test.total_requests} requests over {phase['duration_minutes']} minutes...")
                results = await asyncio.gather(*tasks, return_exceptions=True)
            ...
            if load_test.fire_and_forget:
                logger.info(f"   🔥 Phase {i} completed: {success_rate:.1f}% fired successfully in {actual_phase_duration/60:.1f} minutes")
...
async def main():
    ...
    parser.add_argument("--test", default=None, help="Predefined test scenario (light, medium, heavy, burst, sustained, memory_stress, cpu_stress, comparison_pattern, baseline)")
    parser.add_argument("--test-name", default="Evaluation-Aligned Baseline Comparison Test", help="Name of the test")
    parser.add_argument("--description", default="Load pattern aligned with 15-second evaluation windows for HPA vs KEDA vs DQN comparison", help="Test description")
    ...
    parser.add_argument("--fire-and-forget", action="store_true", help="Use fire-and-forget mode (pure async, no response waiting)")

...
class LoadForecaster:
    ...
    def generate_forecast(self) -> Dict[str, any]:
        ...
        try:
            ...
            if not self.lstm_trainer.is_trained and len(feature_matrix) >= min_samples:
                logger.info(
                    f"🔄 LSTM not trained but have {len(feature_matrix)} data points (min: {min_samples}) - starting training")
            ...
            if self.lstm_trainer.is_trained:
                ...
                if predictions is not None:
                    ...
                    forecast_result["confidence"] = min(0.9, forecast_result.get("confidence",
                                                                                 0.5))  # Higher confidence for LSTM
    ...
    def _calculate_forecast_metrics(self, forecast_values: np.ndarray, current_values: np.ndarray) -> Dict[str, float]:
        ...
        cpu_change = (np.mean(forecast_values[:, cpu_rate_idx]) - current_values[cpu_rate_idx]) / (
                    current_values[cpu_rate_idx] + 1e-6)
        memory_change = (np.mean(forecast_values[:, memory_idx]) - current_values[memory_idx]) / (
                    current_values[memory_idx] + 1e-6)
        request_change = (np.mean(forecast_values[:, request_rate_idx]) - current_values[request_rate_idx]) / (
                    current_values[request_rate_idx] + 1e-6)
            ...
            try:
                with self.training_lock:
                    ...
                    if feature_matrix is not None and len(feature_matrix) >= min_samples:
                        logger.info(
                            f"🔄 Starting LSTM retraining with {len(feature_matrix)} samples (min: {min_samples})")
                        self._train_model_async(feature_matrix)

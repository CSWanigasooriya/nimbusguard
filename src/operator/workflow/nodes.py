...
async def generate_forecast_node(state: OperatorState, services: Dict[str, Any], config: Any) -> OperatorState:
    ...
    try:
        ...
        if 'metrics' in services:
            ...
            lstm_features['next_30sec_pressure'] = min(1.0, base_pressure * (1 + forecast_result.get('confidence', 0.5) * 0.1))
            lstm_features['next_60sec_pressure'] = min(1.0, base_pressure * (1 + forecast_result.get('confidence', 0.5) * 0.2))
...
async def calculate_reward_node(state: OperatorState, services: Dict[str, Any], config: Any) -> OperatorState:
    ...
    try:
        ...
        # Use the sophisticated IoT-inspired reward system
        ...
        if buffer_size >= batch_size:
            try:
                ...
                if training_loss is not None:
                    ...
                    logger.info(f"✅ DQN async training completed: loss={training_loss:.6f}, steps={dqn_agent.training_steps}")
                    ...
                    if 'metrics' in services:
                        ...
                        logger.info(f"📊 Epsilon updated in metrics: {dqn_agent.epsilon:.6f} (decay: {config.scaling.dqn_epsilon_decay})")
                else:

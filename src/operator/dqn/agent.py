...
class ProactiveDQNAgent:
    ...
    async def train_async(self) -> Optional[float]:
        ...
        if current_time - self.last_training_time < self.min_training_interval:
            logger.debug(f"Training skipped: {current_time - self.last_training_time:.1f}s < {self.min_training_interval}s interval")
            return None
    ...
    async def load_model(self, model_name: str = "dqn_model.pt") -> bool:
        ...
        try:
            ...
            if checkpoint['config']['state_dim'] != self.state_dim:
                logger.error(f"State dimension mismatch: expected {self.state_dim}, got {checkpoint['config']['state_dim']}")
                return False
            ...
            logger.info(f"Model loaded from MinIO: {model_name} (steps: {self.training_steps}, epsilon: {self.epsilon:.3f})")
            return True
